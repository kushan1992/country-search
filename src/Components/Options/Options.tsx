    ...
    return (
        <ul className="bg-white border border-gray-100 mt-11 max-h-96 overflow-auto absolute z-50 w-[400px]">
            {result.length > 0 && result.map((country: selectedCountry) => {
                return (
                <li key={country?.ccn3} className="pl-8 pr-2 py-1 border-b-2 border-gray-100 relative cursor-pointer hover:bg-yellow-50 hover:text-gray-900 text-ellipsis overflow-hidden" onClick={() => handleSelectedCounty(country)} data-testid="highlightedText">
